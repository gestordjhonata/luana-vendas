...
async function handleTracking({ parsed, eventId, eventTime, env }) {
  ...
  if (metaResult?.status === 'fulfilled' && metaResult.value) {
    ...
    if (v.skipped) { metaResponseBody = `skipped: ${v.skipped}`; }
    else if (v.response) {
  ...
  if (ga4Result?.status === 'fulfilled' && ga4Result.value) {
    ...
    if (v.skipped) { ga4ResponseBody = `skipped: ${v.skipped}`; }
    else if (v.response) {
  ...
  if (googleAdsResult?.status === 'fulfilled' && googleAdsResult.value) {
    ...
    if (v.skipped) { googleAdsResponseBody = `skipped: ${v.skipped}`; }
    else if (v.response) {
      ...
      if (v.response.ok) {
        let partialErr = null;
        try { const p = JSON.parse(googleAdsResponseBody); partialErr = p?.partialFailureError || null; } catch (_) {}
...
async function handleEncharge({ parsed, env }) {
  ...
  const response = await fetch('https://api.encharge.io/v1/people', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-Encharge-Token': env.ENCHARGE_API_KEY },
    body: JSON.stringify({ email, firstName: nameParts.fn, tags: productConfig.enchargeTag }),
  });
  let responseBody = '';
...
async function handleManyChat({ parsed, env }) {
  ...
  const authHeaders = { 'Authorization': `Bearer ${env.MANYCHAT_KEY}`, 'Content-Type': 'application/json' };
  let subscriberId = '', createBody = '';
  const createRes = await fetch('https://api.manychat.com/fb/subscriber/createSubscriber', {
    method: 'POST', headers: authHeaders,
    body: JSON.stringify({ first_name: nameParts.fn, last_name: nameParts.ln, whatsapp_phone: manychatPhone }),
  });
  try { createBody = await createRes.text(); } catch (e) { createBody = `Read error: ${e.message}`; }
  if (!createRes.ok) { return { statusCode: createRes.status, responseOk: 0, responseBody: `createSubscriber failed: ${createBody}` }; }
  try { subscriberId = JSON.parse(createBody).data?.id || ''; } catch (e) { return { statusCode: createRes.status, responseOk: 0, responseBody: `Parse error: ${e.message}` }; }
  if (!subscriberId) { return { statusCode: createRes.status, responseOk: 0, responseBody: `No subscriber_id: ${createBody}` }; }
  ...
  return { statusCode: tagRes.status, responseOk: tagRes.ok ? 1 : 0, responseBody: `createSubscriber: ${createBody} | addTag: ${tagBody}` };
}
...
async function handlePurchaseLog({ parsed, eventId, eventTime, resultMap, env }) {
  ...
  try {
    ...
    console.error('D1 purchase_items error, rolling back', { transactionId, purchaseId, error: e.message });
    try { await env.DB.prepare('DELETE FROM purchase_log WHERE id = ?').bind(purchaseId).run(); } catch (_) {}
  }
}
...
async function sendToMeta({ checkoutData, hashedEm, hashedFn, hashedLn, hashedPh, hashedExternalId, eventId, eventTime, value, currency, productName, contents, env }) {
  ...
  const metaUserData = { client_ip_address: checkoutData.ip_address || '', client_user_agent: checkoutData.user_agent || '' };
  ...
  const customData = { value: parseFloat(value) || 0, currency, content_type: 'product' };
  ...
  const metaPayload = { data: [{ event_name: 'Purchase', event_time: eventTime, event_id: eventId, event_source_url: checkoutData.event_source_url || '', action_source: 'website', user_data: metaUserData, custom_data: customData }] };
  ...
  const response = await fetch(`https://graph.facebook.com/v25.0/${env.META_PIXEL_ID}/events?access_token=${env.META_ACCESS_TOKEN}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: payloadJson });
...
async function sendToGA4({ checkoutData, hashedEm, transactionId, value, currency, ga4Items, env }) {
  ...
  const purchaseParams = { transaction_id: transactionId, value: parseFloat(value) || 0, currency, engagement_time_msec: 100 };
  if (ga4Items && ga4Items.length) purchaseParams.items = ga4Items;
  const ga4Payload = { client_id: gaClientId, events: [{ name: 'purchase', params: purchaseParams }] };
  if (hashedEm) ga4Payload.user_properties = { email: { value: hashedEm } };
  const payloadJson = JSON.stringify(ga4Payload);
  const response = await fetch(`https://www.google-analytics.com/mp/collect?measurement_id=${env.GA4_MEASUREMENT_ID}&api_secret=${env.GA4_API_SECRET}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: payloadJson });
  return { payload: payloadJson, response };
}

async function getGoogleAdsAccessToken(env) {
  const now = Math.floor(Date.now() / 1000);
  if (googleAdsTokenCache.token && googleAdsTokenCache.expiresAt > now + 30) return googleAdsTokenCache.token;
  const resp = await fetch('https://oauth2.googleapis.com/token', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ grant_type: 'refresh_token', client_id: env.GOOGLE_ADS_CLIENT_ID, client_secret: env.GOOGLE_ADS_CLIENT_SECRET, refresh_token: env.GOOGLE_ADS_REFRESH_TOKEN }) });
  if (!resp.ok) return null;
  const data = await resp.json();
  if (!data.access_token) return null;
  googleAdsTokenCache = { token: data.access_token, expiresAt: now + (data.expires_in || 3600) - 60 };
...
function formatConversionDateTime(unixSeconds, offsetString) {
  ...
  const pad = n => String(n).padStart(2, '0');
  return `${shifted.getUTCFullYear()}-${pad(shifted.getUTCMonth() + 1)}-${pad(shifted.getUTCDate())} ${pad(shifted.getUTCHours())}:${pad(shifted.getUTCMinutes())}:${pad(shifted.getUTCSeconds())}${tz}`;
}

async function sendToGoogleAds({ checkoutData, productConfig, hashedEm, transactionId, value, currency, eventTime, env }) {
  if (!env.GOOGLE_ADS_CUSTOMER_ID || !env.GOOGLE_ADS_LOGIN_CUSTOMER_ID || !env.GOOGLE_ADS_DEVELOPER_TOKEN || !env.GOOGLE_ADS_CLIENT_ID || !env.GOOGLE_ADS_CLIENT_SECRET || !env.GOOGLE_ADS_REFRESH_TOKEN) {
    return { skipped: 'missing google ads env', payload: null, response: null };
  }
  if (!productConfig?.googleAdsConversionActionId) return { skipped: 'no conversion action configured', payload: null, response: null };
  const gclid = checkoutData.gclid || '';
  const wbraid = checkoutData.wbraid || '';
  const gbraid = checkoutData.gbraid || '';
  if (!gclid && !wbraid && !gbraid) return { skipped: 'no click id', payload: null, response: null };
  const accessToken = await getGoogleAdsAccessToken(env);
  if (!accessToken) return { skipped: 'oauth token unavailable', payload: null, response: null };
  ...
  const conversion = { conversionAction: `customers/${customerId}/conversionActions/${productConfig.googleAdsConversionActionId}`, conversionDateTime: formatConversionDateTime(eventTime, env.TIMEZONE_OFFSET), conversionValue: parseFloat(value) || 0, currencyCode: currency || 'BRL', orderId: String(transactionId || '') };
  if (gclid) conversion.gclid = gclid;
  else if (wbraid) conversion.wbraid = wbraid;
  else if (gbraid) conversion.gbraid = gbraid;
  if (hashedEm) conversion.userIdentifiers = [{ hashedEmail: hashedEm }];
  const body = { conversions: [conversion], partialFailure: true, validateOnly: false };
  const payloadJson = JSON.stringify(body);
  const response = await fetch(`https://googleads.googleapis.com/v21/customers/${customerId}:uploadClickConversions`, { method: 'POST', headers: { 'Authorization': `Bearer ${accessToken}`, 'developer-token': env.GOOGLE_ADS_DEVELOPER_TOKEN, 'login-customer-id': loginCustomerId, 'Content-Type': 'application/json' }, body: payloadJson });
  return { payload: payloadJson, response };
}
